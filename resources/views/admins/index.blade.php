...
@section('page-subtitle', 'Kelola akun admin yang dapat mengakses dashboard penuh')
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-700">
        <form method="GET" action="{{ route('admins.index') }}" class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-48">
                <label class="block text-xs font-medium text-slate-500 mb-1">Cari Admin</label>
                <input type="text" name="search" id="search-realtime"
                       ...
                       class="w-full px-3 py-2.5 text-sm border border-slate-300 dark:border-slate-600 rounded-xl
                              bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500">
            </div>
            <button type="submit"
                    class="px-4 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold rounded-xl transition-colors">
                Filter
            </button>
            ...
            <a href="{{ route('admins.create') }}"
               class="px-4 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold rounded-xl transition-colors flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        <div class="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-700">
            <p class="text-sm text-slate-500 dark:text-slate-400">
                Menampilkan <strong class="text-slate-700 dark:text-slate-300">{{ $admins->count() }}</strong>
                dari <strong class="text-slate-700 dark:text-slate-300">{{ $admins->total() }}</strong> admin
            </p>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-full bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center shrink-0">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form method="POST" action="{{ route('admins.destroy', $adm) }}" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            data-confirm="Hapus admin '{{ $adm->name }}'? Aksi ini tidak bisa dibatalkan."
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-16 text-center">
                            ...
                            <p class="text-slate-500 dark:text-slate-400 font-medium">Belum ada admin</p>
                            <a href="{{ route('admins.create') }}"
                               class="mt-4 inline-block px-5 py-2 bg-purple-600 text-white rounded-xl text-sm font-semibold hover:bg-purple-700">
