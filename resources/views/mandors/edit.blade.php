...
@section('page-title', 'Edit Mandor')
@section('page-subtitle', 'Perbarui informasi akun mandor')
...
<div class="max-w-xl mx-auto">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-5">
            <h2 class="text-lg font-bold text-white">Edit Mandor</h2>
            <p class="text-amber-100 text-sm mt-1">{{ $mandor->name }}</p>
        </div>

        <form method="POST" action="{{ route('mandors.update', $mandor) }}" class="p-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $mandor->name) }}"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $mandor->email) }}"
                           class="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-900 text-slate-800 dark:text-white
                ...
                <div>
                    ...
                    <input type="text" name="username" value="{{ old('username', $mandor->username) }}"
