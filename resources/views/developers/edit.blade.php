...
@section('page-subtitle', 'Perbarui data akun developer')
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
        ...
        <div class="px-6 py-5" style="background: linear-gradient(135deg, #1e3a8a, #2563eb);">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-lg font-bold text-white">Edit Developer</h2>
                    <p class="text-blue-100 text-sm mt-0.5">{{ $developer->name }}</p>
                </div>
            </div>
        </div>

        <form method="POST" action="{{ route('developers.update', $developer) }}" class="p-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="username" value="{{ old('username', $developer->username) }}"
                           class="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-900 text-slate-800 dark:text-white
